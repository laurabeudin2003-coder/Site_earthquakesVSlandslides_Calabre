...
st.write("5) Nettoyage des colonnes en cas de valeurs manquantes")
...
ft = pd.DataFrame({
    "Variables": ["Fenêtre temporelle utilisée (en j)", "Rayon de calcul autour de l'épicentre (en km)"],
...
st.markdown("##### Conclusion intermédiaire")
st.write("**Résultats** : Le modèle RandomForestClassifier ne détecte qu’un faible nombre de cas positifs (≈ 8 sur 4000).")
st.write("**Observation** : La forte déséquilibration des classes empêche le modèle d’apprendre efficacement.")
st.write("**Évaluation :**")
st.write("        - Précision (accuracy) élevée → le modèle lit bien les données.")
st.write("        - Rappel (recall) très faible → il ne prédit pas les glissements.")
st.write("**Interprétation** : Le modèle confirme que les séismes déclenchent rarement des glissements de terrain.")
st.write("**Conclusion** : Une relation de causalité existe, mais elle reste faible et indirecte.")
st.write("**Limite** : Le modèle manque de données géophysiques et géologiques pour affiner l’analyse.")
